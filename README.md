...
**You choose what it may touch**, per installation, under Settings → Devices & services → Hearth AI → Configure. Controlling devices and running scenes/scripts are **off by default**. Only entities you exposed to Assist are ever visible — and only ones you exposed can be operated, including scenes and scripts. Home Assistant exposes scenes by default but **not scripts**, so a script you want Hearth to run has to be exposed under Settings → Voice assistants → Expose. A scene that sets a lock, alarm, or camera is refused however it was written.
